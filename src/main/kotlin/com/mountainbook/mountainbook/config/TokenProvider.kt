...
class TokenProvider(
    ...
    fun setTokenExpirationPastValue(token: String): String{
        val claims = Jwts.parserBuilder().setSigningKey(secret).build().parseClaimsJws(token).body
        var now = Date(System.currentTimeMillis())

        return Jwts.builder()
            .setExpiration(Date(now.time - 1000))
